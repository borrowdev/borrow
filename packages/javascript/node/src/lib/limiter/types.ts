...
export interface FixedLimiter extends Record<string, unknown> {
  ...
  /**
   * The interval in which requests are counted. If a number, treated as
   * seconds.
   */
  interval: Interval;
}
...
export interface SlidingLimiter extends Record<string, unknown> {
  ...
  /**
   * The maximum number of requests allowed within the interval since the first
   * request.
   */
  maxRequests: number;
...
export interface TokenLimiter extends Record<string, unknown> {
  ...
  /**
   * The interval in which tokens are added to the bucket. If a number, treated
   * as seconds.
   */
  interval: Interval;
}
...
export interface BorrowLimiter extends Record<string, unknown> {
  ...
  /**
   * The timeout in seconds until the borrow expires. This is necessary to
   * prevent a borrow from being open indefinitely.
   */
  timeout: number;
}
...
/**
 * An array of up to 4 limiter objects of unique types. (TS can enforce minimum
 * length via tuple; maximum length remains in documentation.)
 */
...
export interface CommonLimiterOptions {
  ...
  /** The endpoint to use for the Borrow API. Use this option when self-hosting. */
  endpoint?: {
    /**
     * The base URL of the Borrow API.
     *
     * @example
     *   "https://api.borrow.dev/v1";
     */
    baseUrl: string;
    /**
...
export type LimiterParams<T extends Limiters> = {
  ...
  /**
   * Optional settings for the limiter, including API key, failure behavior, and
   * debug options.
   */
...
export type ContainsTokenLimiter<T extends Limiters> =
  Extract<T[number], { type: "token" }> extends never ? false : true;
